...
# __f-strings__
Formatted Strings is a powerful way of creating a templated string and inserting variables into specific positions with the created string. It's following the synthax below: 
...
## Example with `f" anything {`_`variable`_`}"` 
...
✍️**Challenge 1**: Use f-strings to print the following message:
...
print(f"I am currently in semsteer {semester}. ")
print(f"I am taking {number_credits} credits")
```

:::

## Examples `f" anything {`_`python code`_`}"`
...
✍️**Challenge 2**: Given the price of an item and the tax rate, use f-strings to print the price with taxes, **without** creating an additional variable:
*Total: $22.4825*
