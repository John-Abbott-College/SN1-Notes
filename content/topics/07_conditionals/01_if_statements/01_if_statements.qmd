...
Python uses comparison operators and logical operators to ask YES/NO questions. Then, based on the answer it uses **conditional statements** to change the behaviour of the program.



:::{.callout-note title="Note" collapse="false"}
If you haven't read the pages:  
- [Comparison Operators](../comparison_operators/comparison_operators.qmd)  
- [Logical Operators](../logical_operators/logical_operators.qmd) 

Then a short version is provided below. As you start using `if` statements, you'll want to go back and read those pages (these topics go hand-in-hand).
...
A common mistake beginners make is to forget the indentation of the lines of code within the `if` statement

```python
x = 10
if x == 10:
print("It's equal to 10!"). # Error ❌ This line must be indented with a TAB 
```
...
Boolean variables and conditional statements are designed to be used together. Since a `condition` is a boolean, you can simply use any boolean value or variable **as is** in an `if` statement. For instance, consider the boolean `is_raining`. We would like to display a message if this boolean is `True`. One might be tempted to write:
