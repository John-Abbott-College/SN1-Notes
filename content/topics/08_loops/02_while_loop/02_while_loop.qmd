...
Sometimes you may want to stop repeating the loop even when the condition is still True. Using the `break` statement will skip any remaining lines in the loop body and will continue running code that follows the loop:
...
It's less common to need to do this, but you can also skip the rest of the body statements (just like `break`) but still continue the loop. This uses the `continue` statement:
