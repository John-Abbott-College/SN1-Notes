...
- **Explain** the purpose of the `for`, `break` and `continue` statements
- **Write** `for` loops statements using correct syntax
- **Identify** when a for loop is better than typing the same lines of code many times
- **Construct** a for loop using different forms of the `range` function
:::

A `for loop` allows you to repeat lines of code `n` amount of times. Imagine a guessing game coded in python which lets the player guess a number. What if the game would like to let the player have 10 tries. A `for` loop repeats the statement while **counting the "turns"** .
...
- The variable following the _for_ keyword, known under the names **control variable**, **loop variable** or **iteration variable** automatically tracks the loop’s iterations.
- The _in_ keyword defines the sequence of values the control variable will iterate over.
- The loop body can contain almost any Python code you know. i.e. you can have variables, print(), if-else statements...even more `for` loops!  
...
for letter in my_name:
    print(letter)
```
::: {.callout-note .objectives icon="false" title="Advanced Uses" collapse="true"}
**Lists** and **Files** are topics taught later in the course, so you may want to skip these for now and come back later.
...
| 2           |     | I am at index 2           |                                                                                           |
| 1           |     |                           | There are no more numbers in the range sequence, so for loop quits                             |
...
The third argument is an **increment** or **step**. Its value will be added to the control variable at every loop turn (as you may suspect, the **default value of the increment is 1**).
...
Sometimes, you may want to stop your *for loop* from repeating after a certain condition is met. For example, your *for loop* might be looking for a number that satisfies some condition. Once found, you want to stop the loop from going any further.  

To do this, use the `break` statement to escape or **break out** of the loop.  

Any statements inside the body of the *for loop* beyond the `break` statement will be skipped over and the control flow will pick up again after the end of the loop.
...
for i in range(1, 200):
    if (i % secret_factor) == 0:
        print(f"Found a multiple of {secret_factor}!")
        multiple = i    # keep track that we found this multiple
        break           # this will terminate the for loop

    print(f"{i} is not a multiple of {secret_factor}")

print(f"Finished checking numbers starting at 1, and found that {multiple} is a multiple of {secret_factor}")
```

## The continue statement
The `continue` statement is similar to the `break` statement in that it skips the rest of the *for loop* body, but unlike break it does not stop the loop. Instead, it jumps back to the top of the loop for the next number.

### Example continue

```pyodide
# Finding non-multiples of 3 or 5 in a range
for number in range(1, 20):
    # Skip multiples of 3 or 5
    if number % 3 == 0 or number % 5 == 0:
        continue    # stop processing this number, but continue the loop for the next number

    # Process only numbers that pass the filter
...
for number in fib_list:
  print("The next fibonacci number is ", number)
print("all done")
```
...
| 2           |        | The next fibonacci number is 1 |                                                                                                  |
...
#### **Example 2: Sum of All Integers from 1 to `limit`**

This function calculates the sum of all integers from 1 to a given number `limit`.
...
See how we did limit+1 as the *end* of the sequence because we wanted *limit* to be *included*.

**Explanation**:
The function uses a loop to sum up all the integers from 1 to `limit`. When called with `limit = 5`, it returns 15.
...
**Explanation**:
The function loops through numbers from 1 to `limit` and adds the square of each number to the total. For `limit = 4`, the result is 30.
