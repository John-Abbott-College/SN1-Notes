...
# What's a string?
Strings are nothing more than sequences of [unicode](https://blog.unicode.org/2014/06/announcing-unicode-standard-version-70.html) character ('a','b','c',....'%','#',....). Even space `" "` counts as a character. This includes emojis 😀! Each character in a string occupies a specific position or **index**. 

For example in "Hello World", the `'H'` character is at index **0**, the `'e'` is at index 1, the 'l' is at index 2, etc.

<img src="assets/strings_index.png" width=60%/>

:::{.callout-note collapse="true" title="Fun Fact about Unicode"}
There are over 230,000 unicode characters covering over 170 scripts as well as multiple symbols. Traditionally, computers used the ASCII code which had 256 characters including only 95 english focussed printable characters. 
...
Because strings store text, computers treat them as plain text rather than numbers. This means they can’t be used directly in mathematical calculations. Learning how to convert strings to other data types is essential whenever you need to work with numeric data.
...
:::{.callout-important title="Converting a number from a string might crash!!"}
