...
8. Write a function `calculate_force()` which implements Newton's second law taking the `mass` in Kg and `acceleration` in $m/s^2$ of an object and returns the force in Newton. Use type hinting in the function header. Reminder:
